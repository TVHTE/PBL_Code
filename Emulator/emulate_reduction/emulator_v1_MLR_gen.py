...
class CtaxRedEmulator:
    ...
    def train_ctax_path(self, stepsize, number_of_weights):
        ...
        self.lin_train = [self.lin_path[self.lin_path[:, -1] == path[-1]] for path in self.X_train]
        self.lin_train = [path[0] for path in self.lin_train]
        
        # SORT THE lin_train and X_train  
        self.lin_train = sorted(self.lin_train, key=lambda x: x[-1])
        self.X_train = sorted(self.X_train, key=lambda x: x[-1])
        self.lin_train = np.vstack(self.lin_train)
        self.X_train = np.vstack(self.X_train)
                
        delta_cs = self.lin_train - self.X_train
        final_ctax = self.lin_train[:, -1]
        delta_c_norm = delta_cs / final_ctax[:, None]  # BESPREEK DIT
        
        print(len(delta_c_norm))
