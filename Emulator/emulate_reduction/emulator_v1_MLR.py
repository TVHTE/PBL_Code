...
class CtaxRedEmulator:
    ...
    def test_ctax_path(self, test_path):
        ...
        lin_test_paths = np.asarray(lin_test_paths)        
        ...
        print(delta_c_slice)
        delta_c_slice = np.concatenate(delta_c_slice)
        delta_c_slice = delta_c_slice.reshape(-1, self.number_of_weights)
...
#        cur_weights = self.weights.iloc[(self.weights['ctax'] - final_ctax).abs().argsort()[:1]]
        ...
        cur_weights = []
        for ctax in self.X_test[:, -1]:
            cur_weights.append(self.weights.iloc[(self.weights['ctax'] - ctax).abs().argsort()[:1].values])
...
#        test_red = self.y_test - (delta_c_slice @ b)
        ...
        print(delta_c_slice[30], b[30])
        
        test_test = delta_c_slice[30] @ b[30]
        
        print(test_test)
        
        test_red = pd.DataFrame(test_red.flatten(), columns=['test reduction'])
        ...
        test_red['final ctax'] = self.X_test[:, -1].flatten()
